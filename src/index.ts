...
async function main() {
  ...
  if (!apiKey) { logger.error("DP_DEEPSEEK_API_KEY not set"); process.exit(1); }
  ...
  const visionEnabled = process.env.DP_VISION_ENABLED !== undefined
    ? process.env.DP_VISION_ENABLED === "true" : config.plugins.vision.enabled;
  if (visionEnabled) {
    if (process.env.DP_VISION_BASE_URL && process.env.DP_VISION_API_KEY) {
      pluginEngine.register(new VisionPlugin(logger));
    } else {
      logger.warn("Vision enabled but DP_VISION_BASE_URL/DP_VISION_API_KEY not set — skipping");
    }
  }

  const searchEnabled = process.env.DP_SEARCH_ENABLED !== undefined
    ? process.env.DP_SEARCH_ENABLED === "true" : config.plugins.search.enabled;
  if (searchEnabled) {
